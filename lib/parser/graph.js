...
class Graph {
    ...
    replaceNodeByGraph(otherGraph, nodeName) {

        this.getPrevisousNodes(nodeName).forEach(node => {
            otherGraph.connections['S'].forEach(c => this.addConnection(node, c));
            this.connections[node] = this.connections[node].filter(a => a != nodeName);
        })

        this.connections[nodeName].forEach((node) => {
            otherGraph.getPrevisousNodes('E').forEach(c => this.addConnection(c, node))
        })
